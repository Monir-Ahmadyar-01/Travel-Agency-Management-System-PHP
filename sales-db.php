              ...
              <form action="" method="post" class="col-md-2">
                ...
                <form action="" method="post">
                  ...
                  <div class="btn-group print">
                    ...
                    <button class="btn btn-secondary" data-toggle="collapse" data-target="" onclick="show_all();" class=" btn btn-default btn-sm"><span class="fa fa-eye"></span></button>
                  </div>
                </form>
            ...
            <table id="example1" class="table table-bordered table-striped table-hover col-sm-12" dir="ltr">
               ...
               if(isset($_GET["cus_id"]))
               {
                ...
                if(isset($_POST['search_table'])) {
                  $search_input_01 = $_POST["search_input"];
                        ...
                        $sql_query_01 = mysqli_query($connection,"SELECT *,(select full_name from user where sale.user = user.user_id) as u_name, (select full_name from customer  where customer.customer_id = sale.customer) as cs_name FROM sale where sale.user=(select user_id from user where user.full_name ='$search_input_01') or sale.customer=(select customer_id from customer where customer.full_name ='$search_input_01' and sale.customer='$cus_id')");
                        
                } else if(isset($_POST['search_by_date'])) {
                  $from_date = $_POST["from_date"];
                  $to_date = $_POST["to_date"];
                $sql_query_01 = mysqli_query($connection,"SELECT *,(select full_name from user where sale.user = user.user_id) as u_name, (select full_name from customer  where customer.customer_id = sale.customer) as cs_name FROM sale where sale.date between '$from_date' and '$to_date' and sale.customer='$cus_id'");
                } else {
                  $sql_query_01 = mysqli_query($connection,"SELECT *,(select full_name from user where sale.user = user.user_id) as u_name, (select full_name from customer  where customer.customer_id = sale.customer) as cs_name FROM sale where sale.customer='$cus_id'"); // s INNER JOIN customer cu on s.customer = cu.customer_id 
                }
               }
                ...
                $last_row = array();
                  ...
                  $total_com = 0;
                  $total_paid_price = 0;
                  $total_remain_price = 0;

                  while($fetch_01 = mysqli_fetch_assoc($sql_query_01)){
                  
                  $sale_detail = $fetch_01["sale_bill_number"];
                  $sql_query_05 = mysqli_query($connection,"SELECT SUM(comission + price) as total_amount from sale_detail where sale_bill_number =  '$sale_detail'");
              ...
              ?>
                  <tr>
                      ...
                      <td colspan="2"><?php echo $fetch_01['cs_name'] ?></td>
                      <td><?php echo $fetch_05['total_amount']; ?></td>
                      <td><?php
                      $total_paid_price = $total_paid_price + round($fetch_06['total_amount'],2);
                      echo round($fetch_06['total_amount'],2); ?></td>
                      <td><?php
                      $total_remain_price = $total_remain_price + round($fetch_05['total_amount'] - $fetch_06['total_amount'],2);
                      echo round($fetch_05['total_amount'] - $fetch_06['total_amount'],2); ?></td>
                  ...
                  table thead tr th {
                    ...
                    <td colspan="13" class="hiddenRow">
                      <div class="accordian-body collapse view_all"  id="id_<?php echo $count; ?>"> 
                              ...
                              <tbody>
                                ...
                                ?>
                                  <tr>
                                    ...
                                    <td><?php echo $fetch_05["pax_no"]; ?></td>
                                    ...
                                    <td><?php
                                    $total_price = $total_price + $fetch_05["price"];
                                    echo $fetch_05["price"]; ?></td>
                                    <td><?php 
                                    $total_com = $total_com + $fetch_05["comission"];
                                    echo $fetch_05["comission"]; ?></td>
                                    <td><?php echo $fetch_05["discount"]; ?></td>
                                    <td><?php echo $fetch_05["co_name"]; ?></td>
                                    <td><?php
                                    

                                    $sum_total = $fetch_05['price'] + $fetch_05["comission"];
              ...
              <tr>
                    ...
                    <td><?php echo round($total_price,2); ?></td>

                    <td colspan="1">  Total Commission</td>
                    <td><?php echo round($total_com,2); ?></td>

                    <td colspan="1">  Total Price + Commission</td>
                    <td><?php echo round($total_price + $total_com,2); ?></td>
                    
                    <td colspan="1">  Total Paid Price</td>
                    <td><?php echo round($total_paid_price,2); ?></td>

                    <td colspan="1">  Total Remain</td>
                    <td><?php echo round($total_remain_price,2); ?></td>
              </tr>
            </table>
          </div>
          <!-- /.box-body -->
