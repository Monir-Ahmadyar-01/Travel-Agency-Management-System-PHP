...
<?php 
  if(isset($_GET["salary_id"])){
    ...
    $sql_query_0007 = mysqli_query($connection,"select * from salary_pay where pay_id='$salary_id'");
    $fetch_0007 = mysqli_fetch_assoc($sql_query_0007);
    ...
    if(isset($_POST["salary_save"]))
    {
      ...
      $sql_query_0008 = mysqli_query($connection,"update salary_pay set employee_id='$name',payable='$payable',amount='$amount',currency='$currency',rate='$rate',removal_amount='$removal_amount',detail='$detail',date='$date' where pay_id='$salary_id' ");
           ...
           <div class="row">
             ...
             <div class="col-md-12">
                ...
                <select name="name" id="name" class="form-control" style="height: 40px;">
                      <?php 
                          ...
                          while($row = mysqli_fetch_assoc($show_emp_query)){
                            if($fetch_0007["employee_id"] == $row["staff_id"]){
                      ?>
                          <option selected value="<?php echo $row["staff_id"]; ?>"><?php echo $row["name"]; ?></option>
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" value="<?php echo $fetch_0007["amount"]; ?>" id="amount" name="amount" oninvalid="this.setCustomValidity('این اطلاعات الزامی میباشد. لطفا اطلاعات درست وارد کنید')" oninput="this.setCustomValidity('')" required>
              </div>
              <div class="form-group">
                <label for="removalAmount">مقدار برداشت</label> <br>     
                <input type="number" class="form-control" id="removalAmount" value="<?php echo $fetch_0007["removal_amount"]; ?>" name="removalAmount" oninvalid="this.setCustomValidity('این اطلاعات الزامی میباشد.')" oninput="this.setCustomValidity('')">
            </div>
              <div class="form-group">
                <label for="payable">مقدار قابل پرداخت</label>
                <input type="number" class="form-control" id="payable" value="<?php echo $fetch_0007["payable"]; ?>" name="payable" oninvalid="this.setCustomValidity('این اطلاعات الزامی میباشد.')" oninput="this.setCustomValidity('')" required>
              </div>
                  <div class="form-group" style="position: relative;">
                      <label for="monetary_unit">واحد پول: </label>
                      <select name="monetary_unit" name="monetary_unit" id="monetary_unit" onchange="exchaneg();">
                      <?php 
                          $show_emp_query =  mysqli_query($connection,"select * from currency");
                          while($row = mysqli_fetch_assoc($show_emp_query)){
                            if($fetch_0007["currency"] == $row["currency"]){
                      ?>
                          <option selected value="<?php echo $row["currency_id"]; ?>"><?php echo $row["currency"]; ?></option>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="quantity" value="<?php echo $fetch_0007["rate"]; ?>" id="quantity" placeholder="نرخ" oninvalid="this.setCustomValidity('این اطلاعات الزامی میباشد.')" oninput="this.setCustomValidity('')" required>
                    </div>
           
              <div class="form-group">
                <label for="detail">جزئیات</label>
                <textarea type="text" class="form-control " name="detail" id="detail" oninvalid="this.setCustomValidity('این اطلاعات الزامی میباشد.')" oninput="this.setCustomValidity('')" required>
                  <?php echo $fetch_0007["detail"]; ?>
                </textarea>
              </div>
              <div class="form-group">
                <label for="date">تاریخ</label>
                <input type="date" value="<?php echo  $fetch_0007["date"];?>" class="form-control " name="date" id="date" oninvalid="this.setCustomValidity('این اطلاعات الزامی میباشد.')" oninput="this.setCustomValidity('')" required>
              </div>
